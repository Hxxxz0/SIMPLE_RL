...
def _replay_chunk(
    ...
    env = GraspRlEnv(transform, seed=42 + episodes[0], warmup_steps=60)
...
def prepare_dataset(
    ...
    transform = compute_action_transform(action_episodes)
    transform.save(output_dir / "action_transform.npz")

    rng = np.random.default_rng(seed)
    shuffled = np.asarray(episodes)
    rng.shuffle(shuffled)
    splits = {
        "train": sorted(shuffled[:80].tolist()),
        "val": sorted(shuffled[80:90].tolist()),
        "test": sorted(shuffled[90:].tolist()),
    }

    chunks = [list(map(int, chunk)) for chunk in np.array_split(episodes, min(num_workers, len(episodes))) if len(chunk)]
    payload = {
    ...
    if len(chunks) == 1:
        all_reports = _replay_chunk(
            str(dataset_dir), str(output_dir), chunks[0], rows, payload, 0
        )
    else:
        context = mp.get_context("spawn")
        args = [
            (str(dataset_dir), str(output_dir), chunk, rows, payload, worker % 8)
